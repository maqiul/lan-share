...
| 方式 | 适合场景 | 平台 | 性能 |
|------|---------|------|------|
| **挂载为本地磁盘（客户端）** | 日常浏览、拖拽复制，像本地磁盘一样操作 | Windows / Linux / macOS | ⭐⭐⭐⭐ |
| **CLI 命令行** | 脚本自动化、大文件传输 | 全平台 | ⭐⭐⭐⭐⭐ |
| **Web 文件浏览器** | 临时查看、上传下载、全设备通用 | 全平台（浏览器） | ⭐⭐⭐ |
| **Web 管理后台** | 用户/权限/配额/审计管理 | 全平台（浏览器） | ⭐⭐⭐ |

---

## 方式一：挂载为本地磁盘（推荐日常使用）

客户端 `lanshare-client` 已跨平台，三大系统均可将共享目录挂载为本地磁盘：

| 平台 | 底层驱动 | 前提 |
|------|---------|------|
| Windows | WinFsp | 安装 [WinFsp](https://winfsp.dev/rel/) 运行时（约 5MB） |
| Linux | FUSE (libfuse3) | `apt install libfuse3-dev`（或对应发行版包） |
| macOS | macFUSE | 安装 [macFUSE](https://osxfuse.github.io/) |

### 使用

```powershell
# Windows 简易模式（PIN 认证），挂载为 L 盘
...
```bash
# Linux / macOS 简易模式（PIN 认证），挂载到 ~/LanShare
lanshare-client --server 192.168.0.248:18081 --pin 123456 --mountpoint ~/LanShare

# 前台运行（调试用，Ctrl+C 退出）
lanshare-client --server 192.168.0.248:18081 --pin 123456 --foreground

# 账号模式
lanshare-client --server 192.168.0.248:18081 -u 用户名 -p 密码 --mountpoint /mnt/lanshare
```

挂载后：Windows 资源管理器出现 L 盘；Linux/macOS 在挂载点出现目录，像本地磁盘一样拖拽、复制、粘贴。
...
## 方式三：Web 文件浏览器（零安装，全设备通用）

任何设备（电脑 / 手机 / 平板）打开浏览器访问：

```
http://192.168.0.248:18081/files
```

面向普通用户的文件管理界面，权限受登录账号限制（读/写/删除等按用户权限门控）。

**文件浏览器功能：**
- 🔐 PIN 码 / 账号密码登录
- 🧭 面包屑路径导航 + 目录/文件列表（图标区分类型）
- 👁 文件预览（图片/文本/视频/音频/PDF）
- ⬆ 拖拽上传 / 点击上传，大文件自动分片（5MB/片）带进度
- ⬇ 一键下载（流式）
- ✏️ 重命名，🗑 删除（移到回收站），＋ 新建文件夹
- 🖱 右键菜单操作
- 🌙 暗色主题 + 响应式布局（手机可用）

> 文件浏览器页面同样内嵌在服务端二进制里（`include_str!`），单 exe 即可运行，局域网离线可用。
...
> 所有文件操作走 WSP（WebSocket 自研协议）+ REST API，不依赖 WebDAV。
...
┌──────────────────────────────────────────────────┐
│         lanshare-client (跨平台挂载客户端)         │
│                                                  │
│  • 局域网自动扫描发现服务器                         │
│  • PIN / 账号密码认证                             │
│  • Windows: WinFsp 挂载为盘符（L: 盘）+ 系统托盘   │
│  • Linux/macOS: FUSE 挂载为目录 + 守护进程/前台    │
...
│   │   ├── files.rs        # 文件操作 REST API（列目录/上传/下载/预览/分片）
│   │   ├── wsp.rs          # WSP WebSocket 协议（文件操作）
│   │   ├── db.rs           # SQLite 数据库（用户/会话/设置/审计）
│   │   └── discovery.rs    # 局域网 UDP 广播发现
│   └── assets/
│       ├── index.html      # 管理后台界面（编译时内嵌进二进制）
│       └── files.html      # 文件浏览器界面（编译时内嵌进二进制）
└── lanshare-client/        # 跨平台挂载客户端
    ├── Cargo.toml          # 依赖按平台门控（windows/unix）
    ├── build.rs            # 图标资源嵌入 (winres, 仅 Windows)
    ├── assets/
    │   └── lanshare.ico    # 应用图标
    └── src/
        ├── main.rs         # 精简入口（cfg 分发到 win/unix）
        ├── discovery.rs    # 跨平台共享：自动发现 + 配置 + CLI + 日志
        ├── lsp_client.rs   # LSP3 协议客户端（同步封装，跨平台）
        ├── win/            # Windows 平台
        │   ├── mod.rs      # 入口（WinFsp 挂载 + 托盘）
        │   ├── fs.rs       # WinFsp 文件系统实现（读+写回缓存）
        │   └── tray.rs     # 系统托盘（状态+重连+退出）
        └── unix/           # Linux / macOS 平台
            ├── mod.rs      # 入口（FUSE 挂载 + 守护进程 + 信号处理）
            └── fuse_fs.rs  # FUSE 文件系统实现（读+写回缓存）
```
# LanShare v3.0 操作手册
